...
/*--
 ...
 Storyline - Business that runs MNC food trucking running across different cities - bringing POS Sales Raw data into Snowflake and then harmonizing data to analyze which cities are most profitable and within a city, what are the locations that drive the most amount of business. What are the products that consumers most consume? What are the favourite tourist spots within a city and how close is the food truck from that location?
--*/

-- create frostbyte_tasty_bytes database
CREATE OR REPLACE DATABASE frostbyte_tasty_bytes;

-- create raw_pos schema -> Data coming straight away from the food trucks (raw data)
CREATE OR REPLACE SCHEMA frostbyte_tasty_bytes.raw_pos;

-- create raw_customer schema -> Raw data for customers (loyalty information - sell them coupons)
CREATE OR REPLACE SCHEMA frostbyte_tasty_bytes.raw_customer;

-- create harmonized schema -> Create views on top of raw data and bring third party data like geospatial data to see impact
CREATE OR REPLACE SCHEMA frostbyte_tasty_bytes.harmonized;

-- create analytics schema -> create a schema where we create views consumed by Tableau dashboard.
CREATE OR REPLACE SCHEMA frostbyte_tasty_bytes.analytics;

-- create warehouses -> compute required to query views
...
-- create warehouses -> smaller compute for data analysis purposes - small size for optimal performance of BI dashboard.
